...
def _print_banner():
        ...
        banner += Fore.WHITE + "\tHosted on https://www.github.com/ytisf/BirDuster.\n\n" + Style.RESET_ALL
        print(banner)
...
def _fetch_url(url, headers, ssl_verify=True, write_response=False, timeout=DEF_TIMEOUT):
        ...
        if verb:
                print('executing:',url,'URL nr:', CURL, "of", TURL)
        else:
                sys.stdout.write(".")
        #Keep track of Current URL in list
        CURL +=1
        sys.stdout.flush()
...
def parse_arguemnts():
        ...
        parser.add_argument("-v", "--verbosity", action="count", default=0, help="Increase output verbosity: Outputs a log file in case of interupt (BDS+URLnr.log")
...
if __name__ == "__main__":
        try:
                ...
                if verb:
                        # Write output to file with URL nr at interrupt
                        with open("BDS-"+str(CURL)+".log", 'w', newline='') as csvfile:
                                fieldnames = ['Datetime', 'URL', 'StatusCode', 'ResponseSize']
                                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                                writer.writeheader()
                                for item in FOUND:
